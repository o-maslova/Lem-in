...
// 	{
// 		k = 0;
// 		graph->p_arr[i] = (int *)ft_memalloc(sizeof(int) * (tmp->p_val + 1));
// 		// graph->output[i] = (int *)ft_memalloc(sizeof(int) * tmp->p_val);
...
void	make_arrays(t_graph *graph)
{
	...
	tmp = graph->graph;
	graph->arr = (char **)ft_memalloc(sizeof(char *) * graph->rooms);
	while (tmp)
	{
