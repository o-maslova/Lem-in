...
void		add_vertex(t_vert **graph, char **arr, int pos)
{
	static int	x;
	t_vert		*tmp;
	...
	{
		if (tmp->x < x)
			add_vertex_front(graph, tmp);
		else
			add_vertex_back(graph, tmp);
		// if (x < tmp->x)
		x = tmp->x;
	}
	else
...
int			making_links(char *line, t_vert **graph, t_vert	*tmp)
{
	char		**arr;
...
int		list_fulling(int fd, t_vert **graph, char **line, int pos, t_link **path)
{
	char		**arr;
	// t_vert		*path;
	// t_vert		*tmp;
...
void	parsing(int fd, t_vert **graph)
{
	...
	while (get_next_line(fd, &line) > 0)
	{
		if ((ft_strcmp(line, "##start")) == 0)
			i = list_fulling(fd, graph, &line, 1, &path);
		else if ((ft_strcmp(line, "##end")) == 0)
			i = list_fulling(fd, graph, &line, 2, &path);
		else if (!ft_strchr(line, '#') && ft_strchr(line, '-'))
			i = making_links(line, graph, v_tmp);
		else
			i = list_fulling(fd, graph, &line, 0, &path);
		free(line);
		if (i < 0)
			break ;
	}
